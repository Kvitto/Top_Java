...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        List<UserMealWithExcess> mealsX = new ArrayList<>();
        Map<LocalDate, Integer> dayCalories = new HashMap<>();
        for (UserMeal meal : meals) {
            UserMealWithExcess mealX = new UserMealWithExcess(meal.getDateTime(), meal.getDescription(), meal.getCalories(), false);
            LocalDate day = meal.getDateTime().toLocalDate();
            dayCalories.merge(day, meal.getCalories(), Integer::sum);
            if (isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                mealsX.add(mealX);
            }
        }
        for (UserMealWithExcess mealX : mealsX) {
            mealX.setExcess(dayCalories.get(mealX.getDateTime().toLocalDate()) > caloriesPerDay);
        }
        return mealsX;
    }
